## Hi there 👋

**비즈니스 요구사항을 데이터와 로직으로 구현하는 개발자, 김하은**입니다.

Java, Spring 기반 웹 애플리케이션을 개발하며  
데이터 구조와 비즈니스 로직을 중심으로 문제를 해결해왔습니다.  
실제 운영 과정에서 발생한 정합성 문제를 개선하며, 구조의 중요성을 배우고 있습니다.

기능을 구현하는 데 그치지 않고,  
더 나은 구조를 고민하며 꾸준히 성장하는 개발자입니다.

---

## 🚀 Featured Projects

- 🥐 [Lois Management](https://github.com/ZZINOBAK/lois-management)
  : 케이크 예약 및 카페 운영 효율화를 위한 통합 관리 시스템

- 🛒 [Cabbage-Market](https://github.com/ZZINOBAK/cabbage-market)
  : 상품 등록, 거래 및 실시간 알림 기능을 제공하는 중고거래 웹 서비스

---

## 🔗 Portfolio
- 📑 [PPT](https://drive.google.com/file/d/1avv_h2znighq5lFuBJ2TGnc7M38pmqiO/view?usp=drive_link)
- 📂 [Notion](https://meteor-lychee-b9d.notion.site/24f702d886808091b825d1c1e26fcec5?source=copy_link)

---

## ⚡ Tech Stack

**Backend**
Java · Spring Boot · MyBatis · Spring Security  

**Database**
Oracle · MySQL  

**Infra & DevOps**
AWS (EC2, RDS, S3) · GitHub Actions

---

## 📫 Contact
Email: kimha0094@naver.com
